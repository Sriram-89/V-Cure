  ...
  return (
    <div className="rounded-card border border-border bg-surface p-6 shadow-card">
      ...
      {logPurchase.isSuccess ? (
        <div className="mt-3 flex items-center gap-2 text-xs font-bold text-emerald-800 bg-emerald-50 p-2.5 rounded-xl border border-emerald-100">
          <CheckCircle2 className="h-4 w-4 shrink-0 text-emerald-600" aria-hidden="true" />
          Grocery list items saved for shopping. (External delivery provider is unconfigured).
        </div>
      ) : (
        <Button
          type="button"
          className="mt-4 w-full bg-emerald-600 font-bold text-white hover:bg-emerald-700"
          disabled={!store}
