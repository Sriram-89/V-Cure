...
export default function MealDetailPage() {
  ...
  const [markedEaten, setMarkedEaten] = useState(false);
  ...
  return (
    <div className="min-h-screen bg-gray-50 pb-28">
      ...
      <div className="relative h-64 w-full">
        ...
        <Link
          href="/meals"
          className="absolute top-6 left-6 flex h-10 w-10 items-center justify-center rounded-full bg-black/40 text-white backdrop-blur-md hover:bg-black/60 transition-all"
        >
      ...
      <div className="fixed bottom-16 left-0 right-0 z-40 bg-gradient-to-t from-white via-white to-transparent p-4">
        <div className="mx-auto max-w-md">
          <button
            type="button"
            onClick={() => setMarkedEaten(!markedEaten)}
            className={`w-full rounded-2xl py-3.5 text-base font-bold text-white shadow-lg transition-all flex items-center justify-center gap-2 ${
              markedEaten ? "bg-emerald-800" : "bg-emerald-600 hover:bg-emerald-700"
            }`}
          >
            <UtensilsCrossed className="h-5 w-5" />
            {markedEaten ? "✓ Marked as Eaten" : "Mark as eaten"}
          </button>
        </div>
      </div>
    </div>
