...
describe('MedicalHistoryService', () => {
  ...
  beforeEach(async () => {
    prisma = {
      disease: { findFirst: jest.fn().mockResolvedValue({ id: 'dis_1', name: 'Type 2 Diabetes' }) },
      allergyType: { findFirst: jest.fn().mockResolvedValue({ id: 'at_1', name: 'Peanuts' }) },
      medicalCondition: {
        findMany: jest.fn(),
        findFirst: jest.fn(),
        create: jest.fn(),
    ...
    const module: TestingModule = await Test.createTestingModule({
      providers: [
        ...
        { provide: MedicalHistoryRepository, useValue: new MedicalHistoryRepository(prisma as any) },
      ],
    }).compile();
  ...
  describe('medical conditions', () => {
    ...
    it('creates a condition defaulting status to ACTIVE', async () => {
  ...
  describe('allergies', () => {
    it('resolves the allergen against the curated AllergyType master', async () => {
      prisma.allergyType.findFirst.mockResolvedValueOnce({
        id: 'at_shellfish',
        name: 'Shellfish',
      });
      prisma.allergy.create.mockResolvedValueOnce({
        id: 'a1',
        allergyType: { name: 'Shellfish' },
        severity: 'MODERATE',
        reaction: 'hives',
        deletedAt: null,
        createdAt: new Date(),
      ...
      // Canonical: the Safety Engine reads AllergyType, so the allergen is a
      // catalogue reference rather than free text.
      expect(prisma.allergy.create).toHaveBeenCalledWith(
        expect.objectContaining({
          data: expect.objectContaining({
            userId,
            allergyTypeId: 'at_shellfish',
            severity: 'MODERATE',
          }),
        }),
      );
    ...
    it('rejects an allergen absent from the curated catalogue rather than creating one', async () => {
      prisma.allergyType.findFirst.mockResolvedValueOnce(null);
      await expect(
        service.createAllergy(userId, { allergen: 'Moon dust' }),
  ...
  describe('family history', () => {
    it('creates a family history entry', async () => {
      prisma.familyHistory.create.mockResolvedValueOnce({
        id: 'f1',
        relation: 'FATHER',
    ...
    it('throws NotFoundException when deleting an entry not owned by the user', async () => {
      prisma.familyHistory.findFirst.mockResolvedValueOnce(null);
      await expect(
